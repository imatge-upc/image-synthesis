...
class PICAI_slices(Dataset):
    def __init__(self,
        ...
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        self.path_data = '/mnt/gpid08/users/claudia.giardina/ImageSynthesis3D/PICAI/picai_slices.h5'
        self.mode = 'train' 
        ...
        #open HDF5 file once to avoid averhead
        ...
        raw_shape = [self.dataset_len] + list(self._load_raw_image(0).shape)
        if resolution is not None and (raw_shape[2] != resolution or raw_shape[3] != resolution):
            raise IOError('Image files do not match the specified resolution')
        super().__init__(name='picai', raw_shape=raw_shape, **super_kwargs)

    def _load_raw_image(self, raw_idx):
        slice_i = np.array(self.arr_mri[raw_idx]).astype("float32")
        #copy same info into the three channels
        slice_i = 255*((slice_i) / (4464.631))
...
class PICAI_slices_temp(Dataset):
    def __init__(self,
        path,                   # Path to directory or zip.
        resolution      = None, # Ensure specific resolution, None = highest available.
        **super_kwargs,         # Additional arguments for the Dataset base class.
